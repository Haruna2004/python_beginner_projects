...
class RandomComputerPlayer(Player):
    ...
    def get_move(self,game):
        #get a random valid move for our next move
...
class HumanPlayer(Player):
    ...
    def get_move(self,game):
        ...
        while not valid_square:
            ...
            #we're going to try to cast this input to an
            #integer if its not , then we say its invalid  
            #if the spot is not available on board, we also say it is invalid
            try:
                val = int(square)
...
class GeniusComputerPlayer(Player):
    ...
    def minimax(self, state, player):
        ...
        #first we want to check if the previous moves is a winner
        #this is our base case
        if state.current_winner == other_player:
            #we should return the position AND the score because we need to keep
